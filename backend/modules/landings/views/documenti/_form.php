...
<div class="documenti-form">
    ...
    <div class="row">
        ...
        <div class="col-xs-12">
            <h2 class="subtitle-form">Allegati</h2>
        ...
        <div class="row">
            ...
            <?php endif; ?>
            <?= $form->field($model, 'descrizione_breve')->hiddenInput(['value' => 'null'])->label(false) ?>
            <?= $form->field($model, 'documenti_agid_content_type_id')->hiddenInput(['value' => 1])->label(false) ?>
            <!-- AGID FIELD -->
            <?php if ($enableAgid) : ?>
                <div class="col-md-6 col-xs-12">
                    <?=
...
'activeHeader' => 'ui-icon-amos am am-minus-square',
    ...
    $mm = \backend\modules\landings\models\base\PreferenceLandingDocumenti::findOne(['documenti_id' => $model->id]);
    $closeUrl = (!empty($mm))? \yii\helpers\Url::to('/landings/preference-landing/update?id=' . $mm->preference_landing_id): null;
    if (is_null($mm) && Yii::$app->request->get('urlRedirect')) {
        $closeUrl = Yii::$app->request->get('urlRedirect');
    }
    ...
    echo \open20\amos\workflow\widgets\WorkflowTransitionButtonsWidget::widget([
        ...
        'closeButton' => Html::a($closeButtonText, !is_null($closeUrl)? $closeUrl: $appController->getFormCloseUrl($model), ['class' => 'btn btn-outline-primary']),
