    ...
    private function getPostResponse() {
        ...
        if ($this->request->isPost) {
            ...
            if (isset($post[self::FORM_ID_FILED_NAME]) && $data['id'] == $post[self::FORM_ID_FILED_NAME]) {
                ...
                if ($model->load($this->request->post()) && $this->validateForm($cloneModel, $configs)) {
                    ...
                    if (!empty($attachmentsFields) || !$this->already_present($model, $data)) {
                        ...
                        foreach ($model->attributes as $fld => $vlr) {
                            if (in_array($fld, $attachmentsFields)) {
                                ...
                                $fileUrl = \Yii::$app->params['platform']['backendUrl'] . (strpos($fileUrlProvv, '/') === 0 ? $fileUrlProvv : '/' . $fileUrlProvv);
...
//                die;
    ...
    public function validateForm($cloneModel, $configs) {
        if ($cloneModel->load($this->request->post()) && $cloneModel->validate()) {
            return true;
        }
        return false;
    }
