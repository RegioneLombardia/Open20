...
if ($data['video'] && !$data['image']) {
    // Settings
    $style[] = $data['media_background'] ? "background-color: {$data['media_background']};" : '';
    $attrs_video['class'][] = $data['media_blend_mode'] ? "uk-blend-{$data['media_blend_mode']}" : '';
    // Cover
    $class[] = 'uk-cover-container';
    $attrs_video['uk-cover'] = true;
    // Overlay
    $attrs_overlay['style'] = $data['media_overlay'] ? "background-color: {$data['media_overlay']};" : '';
    // Video
    ...
    <div<?= Uikit::attrs($attrs_image, $attrs_section) ?>>
        ...
        <?php endif ?>
            ...
            <div<?= Uikit::attrs($attrs_section_title) ?>>
                <div class="<?= $data['title_rotation'] == 'left' ? 'avb-rotate-180' : '' ?>"><?= ($data['title_visible'] ? $data['title'] : '') ?></div>
            </div>
        </div>
    <?php endif ?>
