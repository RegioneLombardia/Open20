...
// $navItem   The current content model

?>

<script>
    zaa.bootstrap.register('SeoAnalysisController', ['$scope', '$http', '$sce', function($scope, $http, $sce) {

        $scope.toggleModal = function(toggle) {
            if (toggle) {
                $http.get(`api/1/preview-content?itemId=${<?= $navItem->id ?>}`).then(function(response) {
                    const data = response.data;
                    $('#seoAnalysisModal').modal('show');
                    document.getElementById('amos-seo-analyzer')?.dispatchEvent(new CustomEvent('updatedata', {
                        bubbles: true,
                        detail: {
                            'field': 'text',
                    ...
                    document.getElementById("amos-seo-analyzer")?.dispatchEvent(new CustomEvent('analyze'));
                });
            }
        };
                ...
                <div class="block" ng-class="{ 'block-is-hidden': block.is_hidden == 1, 'block-is-virgin' : !block.is_dirty && isEditable() && block.is_dirty_dialog_enabled && !block.is_container, 'block-is-container': block.is_container, 'block-first': $first, 'block-last': $last }" <?php if ($canBlockUpdate) : ?>dnd dnd-model="block" dnd-isvalid="true" dnd-disable-drag-middle dnd-ondrop="dropItem(dragged,dropped,position,element)" dnd-css="{onDrag: 'block--is-dragging', onHover: 'block--drag-hover', onHoverTop: 'block--drag-top', onHoverMiddle: 'block--drag-middle', onHoverBottom: 'block--drag-bottom'}" <?php endif; ?>>
                    <div class="block-toolbar">
                        <div class="toolbar-item">
                            ...
                            <i class="mdi mdi-{{block.icon}} material-icons"></i>
                                ...
                                <div class="card mb-3">
                                    <div class="card-header">
                                        <ul class="nav nav-tabs card-header-tabs">
                                            <li class="nav-item" ng-click="modalMode=1" ng-show="<?= $isAdmin ? 'block.vars.length > 0' : '(block.vars | filter:{ onlyAdmin: false }).length > 0' ?>">
                                                <a class="nav-link" ng-class="{'active' : modalMode==1}" ng-click="modalMode=1"><?= Module::t('view_update_block_tooltip_edit'); ?></a>
                                            </li>
                                            <li class="nav-item" ng-click="modalMode=2" ng-show="<?= $isAdmin ? 'block.cfgs.length > 0' : '(block.cfgs | filter:{ onlyAdmin: false }).length > 0' ?>">
                                    ...
                                    <div class="card-body">
                                        <form class="block__edit" ng-submit="save()">
                                            <div ng-if="modalMode==1" ng-repeat="field in block.vars<?= !$isAdmin ? ' | filter:{ onlyAdmin: false }' : '' ?>" ng-hide="field.invisible" class="row">
                                                <div class="col" ng-class="{'bold-form-label': field.required}">
                                                    ...
                                                    <div ng-if="field.hasOwnProperty('image') && field.image != ''"><img ng-src="{{ field.image }}" alt=""></div>
                                            ...
                                            <div ng-if="modalMode==2" ng-repeat="cfgField in block.cfgs<?= !$isAdmin ? ' | filter:{ onlyAdmin: false }' : '' ?>" ng-hide="cfgField.invisible" class="row">
                                                <div class="col" ng-class="{'bold-form-label': cfgField.required}">
                                                    ...
                                                    <div type="input-hint"><i class="material-icons">info</i>{{ cfgField.description }}</div>
                                                </div>
                                            </div>
...
<?php // $this->render('_navitem_settings'); 
?>
<div class="cmsadmin-nav-tabs" ng-if="loaded">
    <ul class="nav nav-tabs flex-no-wrap mr-auto" role="tablist">
        <li class="nav-item nav-item-alternative nav-item-icon mr-0">
        ...
        <?php endif; ?>
        <li class="nav item nav-item-title mr-0">
            <span>{{ item.title }}</span>
        </li>
        <li class="nav-item nav-item-alternative nav-item-icon" ng-show="isTranslated">
            <a ng-href="{{cmsConfig.previewUrl}}?itemId={{item.id}}&version={{currentPageVersion}}" target="_blank" title="Visualizza anteprima" class="nav-link btn btn-outline-primary border-0" ng-show="!liveEditState">
                <i class="material-icons">open_in_new</i>
                <span class="sr-only"><?= Module::t("Anteprima"); ?></span>
            </a>
            <a ng-click="openLiveUrl(item.id, currentPageVersion)" ng-show="liveEditState" class="nav-link btn btn-outline-primary border-0" title="Visualizza anteprima">
                <i class="material-icons">open_in_new</i>
                <span class="sr-only"><?= Module::t("Anteprima"); ?></span>
            </a>
        </li>
    ...
    <ul class="nav nav-tabs" role="tablist" ng-if="item.nav_item_type==1 && !isDraft">
    ...
    <?php if ($moduleSeo  && !(Yii::$app->getModule('uikit')->disableAnalizeSeoButton)) : ?>
        <ul class="nav nav-tabs flex-no-wrap" role="tablist">
            <li class="nav-item nav-item-alternative nav-item-icon mr-0" ng-controller="SeoAnalysisController" ng-show="isTranslated">
                <a class="nav-link btn btn-tertiary btn-seo-analyzer rounded" ng-click="toggleModal(1)" title="Analizza il contenuto della pagina">
                </a>
                {{hello.script}}
            </li>
        </ul>
...
<div ng-if="!loaded">caricamento in corso...</div>
