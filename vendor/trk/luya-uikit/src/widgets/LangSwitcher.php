...
class LangSwitcher extends \luya\base\Widget
{
    ...
    /**
     * Return available languages as array with current page link
     *
     * @return array
     */
    public static function asArray() {
        $page = Yii::$app->menu->current;
        $language = Yii::$app->adminLanguage->getActiveLanguage();
        $languages = Yii::$app->adminLanguage->getLanguages();

        $langs = [];
        foreach ($languages as $lang) {
            $viewable = Yii::$app->menu->find()->where(['nav_id' => $page->navId])->lang($lang['short_code'])->with('hidden')->one();
            ...
            /*
                ...
                $lang['link'] = $lang['short_code'] . $item['link'];
            }
            */
        ...
        if (self::$_dataArray === null) {
            ...
            if(count($languages) > 1) {
                foreach ($languages as $language) {
                    $array[] = [
                        'lang' => $language,
                        'item' => Yii::$app->menu->find()->where(['nav_id' => $currentMenuItem->navId])->lang($language['short_code'])->with('hidden')->one(),
                    ];
                }
            }
