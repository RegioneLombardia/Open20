...
class SiteManagementSliderElem extends \amos\sitemanagement\models\base\SiteManagementSliderElem
{
     ...
     * @return mixed
     */
    public static function getUrlEmbedVideoStatic($url, $hideWatchOnYoutube=false){
        $elem = new SiteManagementSliderElem();
        return $elem->getUrlEmbeddedVideo($url, $hideWatchOnYoutube);
    }
     ...
     */
    public function getUrlEmbeddedVideo($urlVideo = null, $hideWatchOnYoutube=false){
        $url = $this->url_video;
        if(!empty($urlVideo)){
            $url = $urlVideo;
        }
