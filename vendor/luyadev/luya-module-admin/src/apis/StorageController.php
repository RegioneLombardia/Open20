...
class StorageController extends RestController
{
     ...
     * @return array
     */
    public function actionDataFiles($folderId = 0, $search = null, $onlyImages = false)
    {
        ...
        if (!empty($onlyImages)) {
            $query->andWhere(['in', 'extension', ['jpg', 'jpeg', 'png', 'svg',
                'tif', 'tiff']]);
        }
    ...
    public function actionFilesUpload()
    {
        foreach ($_FILES as $k => $file) {
            ...
            $errorMessage = $this->checkExtensionErrors($file);
            if(!empty($errorMessage)){
                Yii::$app->response->setStatusCode(422, 'Data Validation Failed.');
                return ['upload' => false, 'message' => $errorMessage, 'file' => null];
            }
    ...
    public function checkExtensionErrors($file){
        $module = \Yii::$app->getModule('attachments');
        if($module  && $module->whiteListExtensions) {
            $whitelistExtensions = implode(',', $module->whiteListExtensions);
            $explode = explode('.', $file['name']);
            if (count($explode) == 2) {
                $extension = $explode[1];
                if (!in_array($extension, $module->whiteListExtensions)) {
                    return "Solo i file con queste estensioni sono permessi: " . $whitelistExtensions;
                }
            }
        }
        return null;
