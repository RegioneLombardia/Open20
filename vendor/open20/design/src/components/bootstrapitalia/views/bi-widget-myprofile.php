        ...
        <?php if (!empty($tags)) : ?>
            <div id="widgetMyProfile" class="<?= $widgetOpened ?>">
                ...
                <div class="collapse-body mt-2 px-0 pt-3 pb-2">
                                    ...
                                    <?php foreach ($tags as $tag) { ?>
                                        ...
                                        $this->render(
                                            ...
                                            [
												...
												'onclick' => 'removeTagMyProfile(' . $tag->id . ',' . $model->user_id . ')',
												'dataconfirm' => 'Sei sicuro di voler rimuovere il tag: ' . urlencode($tag->nome),
					...
					<div class="chip my-1 h-100">
					<a href="<?=  $urlTagManager ?>" title="Aggiungi tag" data-toggle="tooltip" class="d-flex text-decoration-none align-items-center">
						<ul class="chip-group-stacked mx-1">
							<li>
								<svg class="avatar-icon border border-white icon icon-xs">
									<use xlink:href="<?= $currentAsset->baseUrl ?>/sprite/material-sprite.svg#plus-circle"></use>
								</svg>
							</li>
						</ul>
						<span class="chip-label h-100 pt-1">Aggiungi tag</span>
					</a>
				</div>
                    <?php if (empty($adminModule->disableSocialInteraction)): ?>
                                ...
                                <div class="box-last-contents">
                                    <div class="row h-100">
                                        ...
                                        <div class="col-lg-6 d-md-flex justify-content-md-end align-items-end">
                                            ...
                                            ?>
                                            <a class="btn btn-xs btn-outline-secondary btn-icon rounded-circle" title="<?= Module::t('amosdesign', 'Visualizza tutti gli utenti') ?>" style="padding:6px 6px; margin-left:8px;" href="/amosadmin/user-profile/index">
                                                    <span class="sr-only"><?= Module::t('amosdesign', 'Visualizza tutti gli utenti') ?></span>
                                                    <svg class="icon <?= ($disableThemeLight) ? 'icon-white' : '' ?>">
                                                    <use xlink:href="<?= $currentAsset->baseUrl ?>/sprite/material-sprite.svg#magnify"></use>
                                                    </svg>
                                                </a>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-9 d-flex flex-wrap justify-content-end align-items-end">
                                            <?php foreach ($communities as $i => $community): ?>
                                                ...
                                                <?php if($i == $maxCommunity - 1) break; ?>
                                            <?php endforeach; ?>
                                            <div class="community-image rounded ml-1 ml-md-3">
                                                    <a class="btn btn-outline-secondary btn-icon" title="<?= Module::t('amosdesign', 'Visualizza tutte le community') ?>" href="/community/community/index" style="width: 70px;height: 50px;">
                                                        <span class="sr-only"><?= Module::t('amosdesign', 'Visualizza tutte le community') ?></span>
                                                        <svg class="icon icon-lg <?= ($disableThemeLight) ? 'icon-white' : '' ?>">
                                                        <use xlink:href="<?= $currentAsset->baseUrl ?>/sprite/material-sprite.svg#magnify"></use>
                                                        </svg>
                                                    </a>
                                                </div>
