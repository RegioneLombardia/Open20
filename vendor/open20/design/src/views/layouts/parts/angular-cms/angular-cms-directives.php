...
$this->registerJs("  
        function showModal2() {
            $('#luyaloadingstateng').show();
        }
    setTimeout(showModal2, 500);
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="seoAnalysisModalLabel">Analisi SEO</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <?= \open20\amos\seo\widgets\AnalyzerWidget::widget(['options' => ['class' => 'modal-in-cms']]) ?>
            </div>
        </div>
    </div>
</div>
...
<div id="luyaloadingstateng" style="display:none;">
    ...
    <div>
        ...
        <div class="form-group form-side-by-side">
            ...
            <div class="form-side">
                ...
                <label for="{{uid}}redirect_internal"
                       ng-click="data.type = 1"><?= Admin::t('view_index_redirect_internal'); ?></label>

                <input type="radio" ng-model="data.type" ng-value="2" id="{{uid}}redirect_external">
                <label for="{{uid}}redirect_external"
                       ng-click="data.type = 2"><?= Admin::t('view_index_redirect_external'); ?></label>

                <input type="radio" ng-model="data.type" ng-value="3" id="{{uid}}to_file">
                <label for="{{uid}}to_file"
                       ng-click="data.type = 3"><?= Admin::t('view_index_redirect_file'); ?></label>

                <input type="radio" ng-model="data.type" ng-value="4" id="{{uid}}to_mail">
                <label for="{{uid}}to_mail" ng-click="data.type = 4"><?= Admin::t('indirizzo e-mail'); ?></label>
...
<script type="text/ng-template" id="menuDropdownReverse">
    ...
    <ul class="treeview-items">
        <li class="treeview-item"
            ng-class="{'treeview-item-has-children' : (menuData.items | menuparentfilter:container.id:data.id).length}"
            ng-repeat="data in menuData.items | menuparentfilter:container.id:data.id track by data.id"
            ng-include="'menuDropdownReverse'"></li>
    </ul>
</script>
        ...
        <i class="material-icons">delete</i>
            ...
            <modal is-modal-hidden="modal.state" modal-title="<?= Admin::t('layout_select_file'); ?>">
                <div ng-if="!modal.state">
                    <storage-file-manager selection="true" only-images-c="{{onlyImagesC}}"></storage-file-manager>
                </div>
            </modal>
        <?php else: ?>
...
<script type="text/ng-template" id="storageImageUpload">
    <div class="imageupload">
        <div ng-if="imageNotFoundError" class="alert alert-danger" style="margin-top:0px;">The requested image id
            ({{ngModel}}) could not be found anymore. The orignal file has been deleted in the filemanager!
        </div>
        ...
        <div class="imageupload-preview" ng-show="imageinfo != null">
            ...
            <div class="imageupload-infos">
                <div class="imageupload-size" ng-show="!imageLoading">{{ imageinfo.resolution_width }} x
                    {{ imageinfo.resolution_height }}
                </div>
            </div>
        </div>
        <div class="imageupload-upload">
            <storage-file-upload ng-model="fileId" only-images-c="true"></storage-file-upload>
        </div>
        <?php if (Yii::$app->adminuser->canRoute('admin/storage/index')): ?>
            <div class="imageupload-filter" ng-show="!noFilters() && imageinfo != null">
                <select name="filterId" ng-model="filterId" ng-change="changeFilter()" convert-to-number>
                    ...
                    <option ng-repeat="item in filtersData" value="{{ item.id }}">{{ item.name }} ({{ item.identifier
                        }})
                    </option>
                </select>
            </div>
...
<script type="text/ng-template" id="reverseFolders">
    <div class="folders-folder" ng-init="editFolderLabel = false; oldFolder = folder.name"
         ng-class="{'folders-folder--edit': editFolderLabel && !showFoldersToMove, 'folders-folder--move-to': showFoldersToMove, 'folders-folder--undeletable': folder.subfolder}"
         tooltip tooltip-expression="folderCountMessage(folder)" tooltip-position="right">
    ...
    <div class="folder-label" ng-class="{'is-active' : hasFolderActiveChild(folder.id)}">{{folder.name }}</div>
        ...
        <div class="folder-right folder-action-default">
            <button class="folder-button folder-button--edit" ng-click="editFolderLabel=!editFolderLabel;"><i
                        class="material-icons">edit</i></button>
            <button class="folder-button folder-button--delete" ng-hide="folder.subfolder || folder.filesCount > 0"
                    ng-click="deleteFolder(folder)"><i class="material-icons">delete</i></button>
        </div>
        <div class="folder-right folder-action-edit">
            <button class="folder-button folder-button--save"
                    ng-click="updateFolder(folder); editFolderLabel=!editFolderLabel"><i
                        class="material-icons">check</i></button>
            <button class="folder-button folder-button--abort"
                    ng-click="cancelFolderEdit(folder, oldFolder); editFolderLabel=!editFolderLabel;"><i
                        class="material-icons">cancel</i></button>
        </div>
        <div class="folder-right folder-action-move-to">
            <button class="folder-button folder-button--move-to" ng-click="moveFilesTo(folder.id)"><i
                        class="material-icons">subdirectory_arrow_left</i></button>
        </div>
    </div>
    <ul class="folders" ng-if="folder.subfolder === true && folder.toggle_open==1">
        <li class="folders-folder-item" ng-class="{'is-movable' : showFoldersToMove}"
            ng-repeat="folder in getFolderData(folder.id) track by folder.id" ng-include="'reverseFolders'"></li>
    </ul>
</script>
...
<script type="text/ng-template" id="storageFileManager">
        ...
        <div class="filemanager-folders">
            <?php if ($isAdmin): ?>
                <div class="filemanager-add-folder">
                    <div class="btn btn-icon btn-add-folder btn-success" ng-click="folderFormToggler()"
                         ng-if="!showFolderForm">
                        <?= Admin::t('layout_filemanager_add_folder'); ?>
                    </div>
                    <div class="filemanager-add-folder-form" ng-if="showFolderForm">
                        <input class="filemanager-add-folder-input" type="text"
                               placeholder="<?php echo Admin::t('layout_filemanager_folder'); ?>"
                               title="<?php echo Admin::t('layout_filemanager_folder'); ?>" ng-model="newFolderName"/>
    ...
    <div class="folder-label"><?= Admin::t('layout_filemanager_root_dir'); ?></div>
                    ...
                    <ul class="folders">
                        <li class="folders-folder-item" ng-class="{'is-movable' : showFoldersToMove}"
                            ng-repeat="folder in getFolderData(0) track by folder.id"
                            ng-include="'reverseFolders'"></li>
                    </ul>
                </li>
            </ul>
        </div>
        ...
        <div class="filemanager-files">
            <div ng-if="erroreUpload"  class="alert alert-danger" style="margin-top:0px;">
                {{erroreUpload}}
            </div>
            <div class="filemanager-file-actions">
                <div class="filemanager-file-actions-left"
                     ng-class="{'filemanager-file-actions-left-spacing': selectedFiles.length > 0}">
                    <div spellcheck="false" class="btn btn-icon btn-upload filemanager-upload-file"
                         ngf-enable-firefox-paste="true" ngf-drag-over-class="'dragover'" ngf-drop ngf-select
                         ngf-multiple="true" ng-model="uploadingfiles">
                        <?= Admin::t('layout_filemanager_upload_files'); ?>
                    </div>
                    <input class="filemanager-search" type="text" ng-change="runSearch()" ng-model="searchQuery"
                           placeholder="<?= Admin::t('layout_filemanager_search_text') ?>"/>
                </div>
                <div class="filemanager-file-actions-right" ng-show="selectedFiles.length > 0">
                    <button class="btn btn-icon btn-move" ng-class="{'btn-move-active' : showFoldersToMove}"
                            ng-click="showFoldersToMove=!showFoldersToMove">
            ...
            <div class="filemanager-files-table">
                <ol class="breadcrumb small mb-0 mt-3" ng-show="!searchQuery">
                    <li class="breadcrumb-item"><a
                                ng-click="changeCurrentFolderId(0)"><?= Admin::t('layout_filemanager_root_dir'); ?></a>
                    </li>
                    <li class="breadcrumb-item" ng-repeat="fo in folderInheritance | reverse"><a
                                ng-click="changeCurrentFolderId(fo.id)">{{ fo.name }}</a></li>
                </ol>
    ...
    <i class="material-icons">done_all</i>
                            ...
                            <th>
                                <span ng-if="sortField!='name_original' && sortField!='-name_original'"
                                      ng-click="changeSortField('-name_original')"><?= Admin::t('layout_filemanager_col_name'); ?></span>
                                <div class="table-sorter-wrapper is-active">
                                    <div ng-if="sortField=='name_original'"
                                         class="table-sorter table-sorter-up is-sorting"
                                         ng-click="changeSortField('-name_original')">
                            ...
                            <th>
                                <span ng-if="sortField!='extension' && sortField!='-extension'"
                                      ng-click="changeSortField('-extension')"><?= Admin::t('layout_filemanager_col_type'); ?></span>
                                <div class="table-sorter-wrapper is-active">
                                    <div ng-if="sortField=='extension'" class="table-sorter table-sorter-up is-sorting"
                                         ng-click="changeSortField('-extension')">
                            ...
                            <th>
                                <span ng-if="sortField!='upload_timestamp' && sortField!='-upload_timestamp'"
                                      ng-click="changeSortField('-upload_timestamp')"><?= Admin::t('layout_filemanager_col_date'); ?></span>
                                <div class="table-sorter-wrapper is-active">
                                    <div ng-if="sortField=='upload_timestamp'"
                                         class="table-sorter table-sorter-up is-sorting"
                                         ng-click="changeSortField('-upload_timestamp')">
                            ...
                            <th>
                                <span ng-if="sortField!='file_size' && sortField!='-file_size'"
                                      ng-click="changeSortField('-file_size')"><?= Admin::t('layout_filemanager_col_size'); ?></span>
                                <div class="table-sorter-wrapper is-active">
                                    <div ng-if="sortField=='file_size'" class="table-sorter table-sorter-up is-sorting"
                                         ng-click="changeSortField('-file_size')">
                        ...
                        >
                            <th scope="row" ng-click="toggleSelection(file)">
                                <div class="form-check filemanager-toggle-selection"
                                     ng-class="{'form-check-active': inSelection(file)}">
                                    <input type="checkbox" ng-checked="inSelection(file)" class="form-check-input">
                                    <label></label>
                                </div>
                            </th>
                            <td class="text-center" ng-click="toggleSelection(file)" tooltip tooltip-position="left"
                                tooltip-image-url="{{file.createThumbnailMedium.source}}?{{file.upload_timestamp}}"
                                tooltip-disabled="!file.isImage">
                                <span ng-if="file.isImage"><img class="responsive-img filmanager-thumb"
                                                                ng-src="{{file.createThumbnail.source}}?{{file.upload_timestamp}}"/></span>
                                <span ng-if="!file.isImage"><i class="material-icons custom-color-icon">attach_file</i></span>
                            </td>
                            <td ng-click="toggleSelection(file)" tooltip tooltip-position="left"
                                tooltip-text="{{file.id}}">{{file.name_original | truncateMiddle: 50}}
                            </td>
                            ...
                            <td class="text-right">
                                <button type="button" class="btn btn-icon btn-sm filemanager-info-btn"
                                        ng-click="openFileDetail(file)">
                ...
                <div class="text-center mt-4" ng-show="filesData.length == 0">
                    ...
                    <p ng-show="!searchQuery"
                       class="mb-4 text-muted"><?= Admin::t('layout_filemanager_empty_folder'); ?></p>
                    <div ng-show="!searchQuery" spellcheck="false" class="btn btn-outline-success p-4"
                         ngf-enable-firefox-paste="true" ngf-drag-over-class="'dragover'" ngf-drop ngf-select
                         ngf-multiple="true" ng-model="uploadingfiles">
    ...
    <div class="file-detail-view shadow" ng-class="{'open': fileDetail}">
        ...
        <div class="file-detail-view-head">
            <a class="btn btn-icon btn-download"
               ng-href="{{fileDetailFull.file.href}}?{{fileDetailFull.upload_timestamp}}" target="_blank">Download</a>
            <button type="button" class="btn btn-icon btn-cancel file-detail-view-close"
                    ng-click="closeFileDetail()"></button>
        </div>
        ...
        <div ng-if="fileDetail.isImage" class="mt-3 text-center">
            <modal is-modal-hidden="largeImagePreviewState" modal-title="{{ fileDetailFull.file.name }}">
                <div class="text-center">
                    <img class="img-fluid" alt="{{ fileDetailFull.file.name }}"
                         ng-src="{{fileDetailFull.file.source}}?{{fileDetailFull.upload_timestamp}}"/>
                </div>
            </modal>
            <img class="img-fluid" alt="{{ fileDetail.name }}" ng-click="largeImagePreviewState=!largeImagePreviewState"
                 title="{{ fileDetailFull.name }}" style="border:1px solid #F0F0F0"
                 ng-src="{{fileDetail.createThumbnailMedium.source}}?{{fileDetailFull.upload_timestamp}}"/>
        </div>
        <collapse-container class="mt-3" title="<?= Admin::t('layout_filemanager_detail_details'); ?>">
            <input type="text" class="form-control form-control-sm" readonly select-on-click
                   ng-model="fileDetailFull.source"/>
            <table class="table table-hover table-align-middle mt-3">
                ...
                <tr>
                    ...
                    <td>
                        <a ng-show="fileDetailFolder"
                           ng-click="changeCurrentFolderId(fileDetailFolder.id)">{{fileDetailFolder.name }}</a>
                        <a ng-show="!fileDetailFolder"
                           ng-click="changeCurrentFolderId(0)"><?= Admin::t('layout_filemanager_root_dir'); ?></a>
                    </td>
                </tr>
            ...
            <button type="button" class="btn btn-icon btn-save"
                    ng-click="updateFileData()"><?= Admin::t('layout_filemanager_file_captions_save_btn'); ?></button>
        </collapse-container>
    ...
    <span class="flag-fallback">{{key}}</span>
                ...
                <button type="button" class="btn btn-icon btn-save"
                        ng-click="storeFileCaption(fileDetailFull.file)"><?= Admin::t('layout_filemanager_file_captions_save_btn'); ?></button>
            </form>
        </collapse-container>
