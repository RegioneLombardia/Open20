...
$globalSearchInputName = isset(\Yii::$app->params['layoutConfigurations']['globalSearchInputName']) ? \Yii::$app->params['layoutConfigurations']['globalSearchInputName'] : 'global-search';
$addItems = false;
...
if (!$hideUserMenu && !Yii::$app->user->isGuest) {
    ...
    if (Yii::$app->user->can('ADMIN')) {
        $menuUser .= Html::tag(
            'li',
            Html::a(
                Html::tag('span', Module::t('amosdesign', 'Gestisci profili')),
                ['/' . AmosAdmin::getModuleName() . '/user-profile-classes/index'],
                [
                    'class' => 'list-item p-0',
                    'title' => Module::t('amosdesign', 'Gestisci profili')
                ]
            ),
            [
...
<?php if (!($hideHamburgerMenu) && $alwaysHamburgerMenu) : ?>
    ...
    <div class="modal-always-hamburger-menu <?= ($alwaysHamburgerMenuRight) ? 'modal-always-hamburger-menu-right' : 'modal-always-hamburger-menu-left' ?> modal it-dialog-scrollable fade"
         tabindex="-1" role="dialog" id="alwaysHamburgerMenu">
        <div class="modal-dialog <?= ($alwaysHamburgerMenuRight) ? 'modal-dialog-right' : 'modal-dialog-left' ?>"
             role="document">
...
<div id="headerContent"
     class="it-header-wrapper <?= ($enableHeaderSticky) ? 'it-header-sticky' : 'position-fixed' ?> w-100 z-index-8 shadow-sm">
                            ...
                            <div class="it-header-slim-right-zone">
                                ...
                                <?php if (!$disableSettings && !$disablePlatformLinks && ($ordinamentiDashboard || $gestisciWidget)) : ?>
                                    <div class="nav-item dropdown">
                                        <a class="nav-link dropdown-toggle" href="#" data-toggle="dropdown"
                                           ...
                                           title="<?= Module::t('amosdesign', 'Impostazioni') ?>">
                                            <svg class="icon">
                                ...
                                <?php if (!Yii::$app->user->isGuest) : ?>
                                    <?php if (!empty(\Yii::$app->getModule('chat'))) { ?>
                                        ...
                                        ?>
                                        <div class="nav-item">
                                            <a class="nav-link" href="/site/to-menu-url?url=/messages"
                                    ...
                                    <?php if (\Yii::$app->getModule('myactivities')) : ?>
                                        ...
                                        <div class="nav-item">
                                            <a class="nav-link"
                                               href="/site/to-menu-url?url=/myactivities/my-activities/index"
                                               data-toggle="tooltip" data-placement="bottom"
                                               title="<?= AmosMyActivities::t('amosmyactivities', 'My activities') ?>">
                                                <svg class="icon">
                                    ...
                                    <?php if (\Yii::$app->getModule('exportjobs') && !\Yii::$app->user->isGuest && Yii::$app->user->can('EXPORT_READER')) : ?>
                                        ...
                                        <div class="nav-item d-none d-sm-block">
                                            <a class="nav-link" href="/site/to-menu-url?url=/exportjobs/my-export/index"
                                ...
                                <?php if (!$hideUserMenu) : ?>
                                    ...
                                    <?php if (Yii::$app->user->isGuest) : ?>
                                        ...
                                        $hideLoginRegister = isset(\Yii::$app->params['hideLoginRegister']) && \Yii::$app->params['hideLoginRegister'];

                                        ?>
                                        <div class="it-access-top-wrapper">
                                            <?php if (!$hideLoginRegister) { ?>
                                    ...
                                    <?php else : ?>
                                        <div class="dropdown menu-profile">
                                            <a id="dropdownMenuProfile" href="javascript:void(0)"
                                               class="btn btn-primary btn-icon btn-full dropdown-toggle" role="button"
                                               data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
                                               title="<?= Module::t('amosdesign', 'Apri menu utente') ?>">
                                                <span class="rounded-icon">
                                                    <img class="icon icon-primary rounded-circle"
                                                         src="<?= $userImage ?>" alt="<?= $userAltImg ?>">
                                                </span>
                                ...
                                <?php if (!$hideLangSwitchMenu) : ?>
                                    <div class="nav-item dropdown menu-translation border-left border-white pl-2">
                                        <a class="nav-link dropdown-toggle pr-0" href="javascript::void(0)"
                                           title="<?= Module::t('amosdesign', 'Lingua corrente') . ' ' . $actualLang ?>"
                                ...
                                <?php if (!($hideGlobalSearch)) : ?>
                                    <?php if (!($hideGlobalSearchForGuest)) : ?>
                                        <?php if ($enableGlobalSearchForm) : ?>
                                            <a class="btn btn-xs border-tertiary btn-collapse-search d-block d-sm-none"
                                               data-toggle="collapse" href="#collapseGlobalHeaderForm" role="button"
                                            ...
                                            <?php if (isset($pageSearchLink)) : ?>
                                                <div class="it-search-wrapper">
                                                    ...
                                                    <a class="search-link rounded-icon"
                                                       aria-label="<?= Module::t('amosdesign', 'Cerca') ?>"
                                                       title="<?= Module::t('amosdesign', 'Vai alla pagina di ricerca della piattaforma') . ' ' . \Yii::$app->name ?>"
                                                       href="<?= $pageSearchLink ?>">
                                                        <svg class="icon">
                                                            <use xlink:href="<?= $currentAsset->baseUrl ?>/node_modules/bootstrap-italia/dist/svg/sprite.svg#it-search"></use>
                                                        </svg>
                        ...
                        <div class="row">
                            <div class="col-12">
                                ...
                                <nav class="navbar navbar-expand-lg has-megamenu">
                                    <button class="custom-navbar-toggler" type="button" aria-controls="hamburgerMenu"
