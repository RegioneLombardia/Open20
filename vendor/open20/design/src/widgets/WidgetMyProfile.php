...
class WidgetMyProfile extends Widget
{
    ...
    public function getLastCommunities()
    {
        ...
        foreach ($communities as $community) {
            if($community->context == 'open20\amos\organizzazioni\models\Profilo'){
                $urlLogo = '/img/img_default.jpg';

                $profilo = Profilo::findOne(['community_id' => $community->id]);
                if($profilo && $profilo->logoOrganization){
                    $urlLogo = $profilo->logoOrganization->getWebUrl('item_community', false, true);
                }
            }else if (!empty($community->communityLogo)) {
