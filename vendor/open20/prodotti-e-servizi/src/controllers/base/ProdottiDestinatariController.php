 ...
 * ProdottiDestinatariController implements the CRUD actions for ProdottiDestinatari model.
 *
 * @property \open20\prodottiservizi\models\ProdottiDestinatari $model
 * @property \open20\prodottiservizi\models\search\ProdottiDestinatariSearch $modelSearch
 *
 * @package open20\prodottiservizi\controllers\base
 */
class ProdottiDestinatariController extends CrudController
{
    ...
    {
        $this->setModelObj(new ProdottiDestinatari());
        $this->setModelSearch(new ProdottiDestinatariSearch());
    ...
    /**
     * Lists all ProdottiDestinatari models.
     * @return mixed
     */
    ...
    /**
     * Displays a single ProdottiDestinatari model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new ProdottiDestinatari model.
    ...
    public function actionCreate()
    {
        ...
        $this->model = new ProdottiDestinatari();
    ...
    /**
     * Creates a new ProdottiDestinatari model by ajax request.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
...
//Yii::$app->getSession()->addFlash('danger', BaseAmosModule::t('amoscore', 'Item not created, check data'));
    ...
    /**
     * Updates an existing ProdottiDestinatari model.
    ...
    /**
     * Deletes an existing ProdottiDestinatari model.
    ...
    public function actionDelete($id)
    {
        $this->model = $this->findModel($id);
        if ($this->model) {
            $mmToDelete = ProdottiServiziDestinatariMm::find()
                ->andWhere(['destinatario_id' => $id])
                ->all();
            foreach($mmToDelete as $destinatario):
                $destinatario->delete();
            endforeach;
            $this->model->delete();
