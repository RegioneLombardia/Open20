...
use open20\prodottiservizi\models\search\ProdottiFinalitaSearch;
...
/**
 * Class ProdottiFinalitaController
 * ProdottiFinalitaController implements the CRUD actions for ProdottiFinalita model.
 *
 * @property \open20\prodottiservizi\models\ProdottiFinalita $model
 * @property \open20\prodottiservizi\models\search\ProdottiFinalitaSearch $modelSearch
 *
 * @package open20\prodottiservizi\controllers\base
 */
class ProdottiFinalitaController extends CrudController
{
    ...
    {
        $this->setModelObj(new ProdottiFinalita());
        $this->setModelSearch(new ProdottiFinalitaSearch());
    ...
    /**
     * Lists all ProdottiFinalita models.
     * @return mixed
     */
    ...
    /**
     * Displays a single ProdottiFinalita model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new ProdottiFinalita model.
    ...
    public function actionCreate()
    {
        ...
        $this->model = new ProdottiFinalita();
    ...
    /**
     * Creates a new ProdottiFinalita model by ajax request.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
...
//Yii::$app->getSession()->addFlash('danger', BaseAmosModule::t('amoscore', 'Item not created, check data'));
    ...
    /**
     * Updates an existing ProdottiFinalita model.
    ...
    /**
     * Deletes an existing ProdottiFinalita model.
    ...
    public function actionDelete($id)
    {
        $this->model = $this->findModel($id);
        if ($this->model) {
            $mmToDelete = ProdottiServiziFinalitaMm::find()
                ->andWhere(['finalita_id' => $id])
                ->all();
            foreach($mmToDelete as $finalita):
                $finalita->delete();
            endforeach;
            $this->model->delete();
