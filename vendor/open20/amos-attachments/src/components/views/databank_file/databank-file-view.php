...
//JAVASCRIPT DIPENDENTE DALL'ATTRIBUTO
if(!loadedOnce['$attribute']){
    loadedOnce['$attribute'] = 1;

    selected_file_ids['$attribute'] = [];
    selected_file_names['$attribute'] = {};
    //ON OPEN MODAL
    $(document).on('click', '.open-modal-databank-file.$attribute', function (event) {
        resetFileCaption();
        selected_file_ids['$attribute'] = [];
        selected_file_names['$attribute'] = {};
    ...
    // INSERISCI FILE SELEZIONATI
     $(document).on('click', '#insert-files-btn-$attribute', function(e){
         e.preventDefault();
         if(selected_file_ids['$attribute'].length == 0){
             alert('Non hai selezionato alcun file');
             return true;
         }
         let file_ids = selected_file_ids['$attribute'].join();
         var csrf = $('form input[name="$csrfParam"]').val();
         ...
         $.ajax({
          ...
          success: function(data){
              if(data.success == true){
                 ...
                 if(selected_file_ids['$attribute'].length == 0){
                     resetFileCaption();
                 }else if(selected_file_ids['$attribute'].length == 1){
                       $(filecaption).val(Object.values(selected_file_names['$attribute'])[0]);
                 }else {
                       $(filecaption).val(selected_file_ids['$attribute'].length+" Selezionati");
                 }
    ...
    function addRemoveItemSelected(item_file, id_file, attribute, name){
         if($(item_file).hasClass('file-item-selected')){
            ...
            const index = selected_file_ids['$attribute'].indexOf(id_file);
            if (index > -1) {
                selected_file_ids['$attribute'].splice(index, 1);
            }
            $("#sel-file-"+attribute+"-"+id_file).remove();
            $('#label-selezionati-'+attribute).text(labelFileSelezionati+" ("+selected_file_ids['$attribute'].length+")");
            delete selected_file_names['$attribute'][id_file];
        }else{
            $(item_file).addClass('file-item-selected');
            selected_file_ids['$attribute'].push(id_file);
            selected_file_names['$attribute'][id_file] = name;
...
<div id="container-databank-file-<?= $attribute ?>">
    ...
    <div id="file-databank-file-<?= $attribute ?>" class="file-databank-list">
        <?php
        $dataProvider = $modelSearch->searchGenericFiles(['extensions' => $extensions]);
