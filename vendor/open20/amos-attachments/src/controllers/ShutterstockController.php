...
//        $sub = $shutterstockClient->getSubscriptions();
//        $license = $shutterstockClient->getProscriptions();
//pr($id);
//        pr($sub);
//pr($license,'FFF');
//        $a= [
...
//                 ]
//             ];
//        $license = $shutterstockClient->buyProscriptions($a);
...
//                pr($resp);
     ...
     * @return array
     */
    public function actionUploadFromGalleryAjax($id, $attribute = null, $license_type = null, $name = null, $csrf = null)
    {
        try {
            ...
            $isBought = false;
            $licenses = $shutterstock->getProscriptions(['image_id' => $id]);
            $msg = $shutterstock->errorMessage;
            $activeProscription = Shutterstock::activeProscriptionForImage($licenses, $license_type);
            ...
            if (empty($activeProscription)) {
                ...
                $licenses = $shutterstock->buyProscriptions($params);
                if (!empty($licenses->data)) {
                    $activeProscription = $licenses->data[0];
