...
class CwhUtil {
    ...
    public static function checkCwhScope($cwhScope) {
        if (
            ...
            if (isset($id[0][0])) {
                ...
                $cwhScope['community'] = $id > 0 ? $id : null;
            }
        }
     ...
     * @param bool $onlyForProfile
     * @return array
     * @throws \yii\base\InvalidConfigException
     */
    public static function findInterestTagIdsByUser($userProfileId, $interestClassname = 'simple-choice', $onlyForProfile = false) {
        $count = 0;
        ...
        if ($onlyForProfile == true) {
            $roots = \Yii::$app->db->createCommand("select distinct(tag_id) as tag_id from cwh_tag_interest_mm where classname like '%UserProfile'")->queryColumn();
            $query->andWhere(['root_id' => $roots]);
        }
     ...
     * @throws \yii\base\InvalidConfigException
     */
    public static function findInterestTagsByUser($userProfileId, $interestClassname = 'simple-choice', $onlyForProfile = false) {
        /** @var AmosTag $tagModule */
        $tagModule = AmosTag::instance();
        $tagModel = $tagModule->createModel('Tag');
        $userInterestTagIds = CwhUtil::findInterestTagIdsByUser($userProfileId, $interestClassname, $onlyForProfile);
