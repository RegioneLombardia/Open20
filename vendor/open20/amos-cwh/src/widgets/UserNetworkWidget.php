...
class UserNetworkWidget extends Widget {
    ...
    public function run() {
        ...
        if (!empty(\Yii::$app->getModule('organizzazioni'))) {
            if (!in_array('open20\amos\organizzazioni\models\Profilo', $already)) {
                $networkObject = \Yii::createObject('open20\amos\organizzazioni\models\Profilo');
                if ($networkObject->hasMethod('getUserNetworkWidget')) {
                    $html .= $networkObject->getUserNetworkWidget($this->userId, $this->isUpdate);
                }
            }
