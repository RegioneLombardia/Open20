...
//                    ->limit(100)
            ...
            foreach ($networks as $network) {
                if ($scopeFilter) {
                    ...
                    if ($enableDestinatariFatherChildren && isset($this->scope[$networkModel['tablename']]) && (
                        ...
                        if (!empty($network['classname']) && !empty($network['record_id'])) {
                            $obj = $network['classname']::findOne($network['record_id']);
                            if (!empty($obj) && $obj->hasProperty('name')) {
                                $network['text'] = $obj->name;
                            }
                        }
    ...
    public function renderWithoutScope($destinatariData, $labelSuffix) {
        ...
        $primo_piano_checkbox = Yii::$app->user->can('UROI_LANDING') ? (Yii::$app->getModule('news')->params['site_publish_enabled'] ? Html::checkbox(
        ...
        if ($this->getModel()->isNewRecord && $this->moduleCwh->enabelDefaultByPrevalentPartnership == true) {
            $userProfile = \open20\amos\admin\models\UserProfile::find()->andWhere(['user_id' => \Yii::$app->user->id])->one();
            $moduleOrg = \Yii::$app->getModule('organizzazioni');
            if (!empty($userProfile) && !empty($userProfile->prevalent_partnership_id) && !empty($moduleOrg)) {
                $org = \open20\amos\organizzazioni\models\Profilo::findOne($userProfile->prevalent_partnership_id);
                $key = key($destinatariData);
                if (!empty($org) && !empty($key)) {
                    $destinatariData[$key]['profilo-' . $org->id] = $org->name;
                    $this->getModel()->destinatari = 'profilo-' . $org->id;
                    $defaultProf = 'profilo-' . $org->id;
                    $this->view->registerJs("
                            $('#cwh-destinatari').val('{$defaultProf}');
                            $('#cwh-destinatari').trigger('change.select2');
                            console.log('ok');
