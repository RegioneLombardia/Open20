...
use open20\amos\emailmanager\models\EmailSpamBlock;
...
class AmosEmailManager implements ManagerInterface
{
    ...
    /**
     * Check if the user reached the max number of emails per minute
     * @param $senderUserId int
     * @param $maxMailsPerMinute int
     * @return bool
     */
    public function checkSpamBlock($senderUserId, $maxMailsPerMinute)
    {
        $canSend = true;
        try {
            $emailSpamBlock = EmailSpamBlock::findOne(['sender_user_id' => $senderUserId]);
            $now = date('Y-m-d H:i:s');
            if (!empty($emailSpamBlock)) {
                $sendingDate = $emailSpamBlock->sending_date;
                $timeDiff = strtotime($now) - strtotime($sendingDate);
                if ($timeDiff > 60) {
                    $emailSpamBlock->counter = 1;
                    $emailSpamBlock->sending_date = $now;
                    $emailSpamBlock->save(false);
                } else {
                    if ($emailSpamBlock->counter >= $maxMailsPerMinute) {
                        $canSend = false;
                        if (Yii::$app instanceof \yii\web\Application) {
                            if (!Yii::$app->request->isAjax) {
                                Yii::$app->session->addFlash('danger', Yii::t('amosemail', 'Hai raggiunto il limite massimo di e-mail al minuto.'));
                            } else {
                                Yii::$app->response->setStatusCode((new \yii\web\TooManyRequestsHttpException)->statusCode);
                                Yii::$app->response->statusText = Yii::t('amosemail', 'Hai raggiunto il limite massimo di e-mail al minuto.');
                                Yii::$app->response->send();
                            }
                        }
                    } else {
                        $emailSpamBlock->counter++;
                        $emailSpamBlock->save(false);
                    }
                }
            } else {
                $emailSpamBlock = new EmailSpamBlock();
                $emailSpamBlock->sender_user_id = $senderUserId;
                $emailSpamBlock->sending_date = $now;
                $emailSpamBlock->counter = 1;
                $emailSpamBlock->save(false);
            }
