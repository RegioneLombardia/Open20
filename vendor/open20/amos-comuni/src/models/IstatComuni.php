...
//public $validatori;
     ...
     */
    public static function getComuniSospesi()
    {
        return self::find(['soppresso' => 1]);
    }

    public static function getComuniAttivi()
    {
        return self::find(['soppresso' => 0]);
    }

    /**
     * @param bool $exlude_sospesi se true esclude i cap sospesi
     * @return array con i cap del comune
     * recupera tutti i CAP associati al comune, escludendo quelli sospesi in base al parametro in input
     */
    public function getComuneCaps($exlude_sospesi=false){
        $return_cap =[];
        foreach ( $this->istatComuniCaps as $k => $istat_cap ){
            //salto l'inserimento ll'array di ritorno se il cap è settato come sospeso
            if($exlude_sospesi && $istat_cap->sospeso==1 ){
                continue;
            }
            $return_cap[] = $istat_cap->cap;
        }
