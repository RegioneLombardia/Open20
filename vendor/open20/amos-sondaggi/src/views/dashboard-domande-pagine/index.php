...
$this->params['titleButtons'][] = Html::a(
    AmosIcons::show('plus-circle') . '&nbsp;' . AmosSondaggi::t('amossondaggi', '#new_page'),
...
function orderupdown() {
	$(document).on("click", '[id*="sotto-"]', function(e){
    e.preventDefault();
    var id = $(this).data('id');
    //console.log(id);
    $.ajax({
        ...
        success: function (data) {
          if (data[0])
            $.pjax.reload({container: '#dashboard-domande-pagine-view', timeout: 10000});
          else showCannotOrderModal();
        }
    });
    
  });
  $(document).on("click", '[id*="sopra-"]', function(e){
      e.preventDefault();
      var id = $(this).data('id');
      $.ajax({
