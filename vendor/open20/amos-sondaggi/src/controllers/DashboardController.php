...
//        $this->setUrl($url);
    ...
    public function actionCreateCommunication($idSondaggio, $url = null)
    {
        ...
        $sondaggio = $this->model->sondaggi;
        $this->setMenuSidebar($sondaggio, $this->model->id);

        if ($this->model->load(Yii::$app->request->post())) {
            $this->model->updateQueryAndCount(false);
            if ($this->model->save()) {
...
//        $this->setUrl($url);
    ...
    public function actionSendCommunications($id, $url, $preview = false)
    {
        ...
        if (!empty($comunicazione)) {
            ...
            $all = \Yii::$app->db->createCommand($comunicazione->query)
                ->queryAll();
            if ($preview == true) {
                if (!$this->sendEmailGeneral($to, $comunicazione->subject,
                    $comunicazione->message)) {
                    $communicationsFailed = [$to];
                }
            } else {
                $communicationsFailed = $this->sendEmails($all, $comunicazione);
                //aggiorno il conteggio della comunicazione
                $comunicazione->count = count($all);
                if (!$comunicazione->save()) {
                    Yii::$app->session->addFlash('danger',
                        AmosSondaggi::t('amossondaggi', 'Errore'
                            . ' nell\'aggiornamento dei dati della'
                            . ' comunicazione'));
                }
            }
            if (empty($communicationsFailed)) {
                \Yii::$app->session->addFlash('success', AmosSondaggi::t('amossondaggi',
                    "Comunicazione inviata correttamente"));
            } else {
                foreach ($communicationsFailed as $communicationFailed) {
                    \Yii::$app->session->addFlash('danger', AmosSondaggi::t('amossondaggi',
                        "Errore nell'invio della comunicazione a {organization}.",
                        ['organization' => $communicationFailed]));
                }
            }
        }
        else
        {
            Yii::$app->session->addFlash('danger', AmosSondaggi::t('amossondaggi',
                'Dati del sondaggio non trovati'));
        }
        return $this->redirect([$url]);
    }

    /**
     * Invia un'email ad uno o piu' destinatari. Come mittente verra' usato il
     * parametro di piattaforma 'email-assistenza'
     * @param string|array $to Destinatario (o destinatari)
     * @param string $subject Oggetto dell'email
     * @param string $message Messaggio dell'email
     * @param array $files File da allegare
     * @param UserProfile $profile Profilo utente del destinatario
     * @return bool Restituisce true in caso di successo o false altrimenti
     */
    protected function sendEmailGeneral($to, $subject, $message, $files = [], $profile = null)
    {
        try {
            ...
            /** @var Email $email */
            $email = new Email();
            return $email->sendMail($from, $to, $subject, $message, $files, [], ['profile' => $profile]);
        } catch (\Exception $ex) {
    ...
    /**
     * Invia la medesima email ad una serie di destinatari. I dati per generare
     * l'email vengono elaborati dalla comunicazione (SondaggiComunication)
     * passata al metodo
     * @param array $all Destinatari delle email. Questo array dovra' essere
     * composto da array che hanno valorizzato con l'indirizzo email del
     * destinatario l'indice 'id' per le comunicazioni di tipo 1 e l'indice
     * 'to_id' per tutte le altre tipologie di comunicazioni
     * @param SondaggiComunication $comunicazione Comunicazione con cui verra'
     * generata l'email da inviare
     * @return array Restituisce i nomi dei destinatari a cui non e' stato
     * possibile inviare l'email. Nel caso tutti gli invii vadano a buon fine
     * verra' restituito un array vuoto
     */
    protected function sendEmails($all, $comunicazione)
    {
        ...
        return (empty($communicationsFailed));
    }
