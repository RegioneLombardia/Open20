...
class SondaggiSearch extends Sondaggi implements CmsModelInterface
{
    ...
    public function attributeLabels()
    {
        return ArrayHelper::merge(parent::attributeLabels(), [
            ...
            'closed' => AmosSondaggi::t('amossondaggi', '#closed'),
            'compilazioniStatus' => AmosSondaggi::t('amossondaggi', '#compilazioniStatus'),
...
//            'status',
    ...
    public function getSearchQuery($query)
    {
        if (!empty($this->status)) {
            if ($this->status == self::STATUS_CONCLUSO) {
                $query->andWhere(['<', new Expression('DATE(close_date)'), date('Y-m-d')])
                    ->andWhere(['status' => self::WORKFLOW_STATUS_VALIDATO]);
            } else {
                $query->andFilterWhere(['status' => $this->status]);
                if ($this->status == self::WORKFLOW_STATUS_VALIDATO) {
                    $query->andWhere(['>=', new Expression('DATE(close_date)'), date('Y-m-d')]);
                }
            }
        }
        if (! empty($this->date_from)) {
            $query->andFilterWhere([
                '>=',
                new Expression("DATE(publish_date)"),
                new Expression("DATE('{$this->date_from}')")
            ]);
        }
        ...
        if (! empty($this->date_to)) {
            $query->andFilterWhere([
                '<=',
                new Expression("DATE(publish_date)"),
                new Expression("DATE('{$this->date_to}')")
            ]);
        }
    ...
    public function searchByUserOrganization($params)
    {
        ...
        if ($compilazioniStatus && $compilazioniStatus != '0') {
            ...
            if (AmosSondaggi::instance()->compilationToOrganization) {
                ...
                $query = $dataProvider->query->andWhere([
                    SondaggiRisposteSessioni::tableName() . '.user_id' => $userId
                ]);
            }
            $dataProvider->query->andWhere([
                SondaggiRisposteSessioni::tableName() . '.status' => $compilazioniStatus
            ]);
        }
        $dataProvider->query->andWhere([
            'sondaggi.status' => Sondaggi::WORKFLOW_STATUS_VALIDATO
        ])
    ...
    public function searchByUserOrganizationOpen($params)
    {
        ...
        $dataProvider->query = $dataProvider->query->andWhere([
            '>=',
            'sondaggi.close_date',
            new Expression('curdate()')
        ]);
    ...
    public function searchDominio($params)
    {
        ...
        $query->andWhere([
            'status' => self::WORKFLOW_STATUS_VALIDATO
        ]);
    ...
    public function ultimiSondaggiLive($params, $limit = null)
    {
        ...
        $dataProvider->query->andWhere([
            'sondaggi.sondaggio_type' => \open20\amos\sondaggi\models\base\SondaggiTypes::getLiveType()
        ])->
    ...
    public function searchAllAdminExtended($params) {
        $dataProvider = $this->search($params, 'admin-scope');
