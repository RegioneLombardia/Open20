...
class ItemAndCardHeaderWidget extends Widget {
    ...
    public function init() {
        ...
        if (!$this->getPublicationDateNotPresent() && !$this->isHideInteractionMenu() && (is_null($this->_publicationDateField) || !is_string($this->_publicationDateField) || !strlen($this->_publicationDateField))) {
...
//            }
    ...
    public function run() {
        ...
        if ($this->showPrevalentPartnership) {
            if ($this->enableLink) {
                $dataOrg = $this->retrievePrevalentPartnership(true);
                $contentToRender = array_merge($contentToRender, [
                    'contentPrevalentPartnership' => '<a href="' . $dataOrg['url'] . '" title="' . urlencode($dataOrg['name']) . '">' . $dataOrg['name'] . '</a>'
                ]);
            } else {
                $contentToRender = array_merge($contentToRender, [
     ...
     * @return string|array
     */
    private function retrievePrevalentPartnership($enable_link = false) {
        $prevalentPartnershipName = "";
        $prevalentPartnershipData = [];
        if (!is_null($this->_contentCreator)) {
            ...
            if (!is_null($prevalentPartnership) && ($prevalentPartnership instanceof OrganizationsModelInterface)) {
                ...
                $okCommunity = $this->canUseCommunity($prevalentPartnership->community_id);
                $urlViewOrg = ((!empty($prevalentPartnership->community_id) && $okCommunity) ? \yii\helpers\Url::to(['/community/join/open-join', 'id' => $prevalentPartnership->community_id]) : \yii\helpers\Url::to(['/organizzazioni/profilo/view', 'id' => $prevalentPartnership->id]));
                $prevalentPartnershipData = ['name' => $prevalentPartnership->getNameField(), 'url' => $urlViewOrg];
            }
        }
        if ($enable_link) {
            return $prevalentPartnershipData;
        }
     ...
     * @return bool
     */
    private function canUseCommunity($id = null) {
        $ret = false;
        $moduleComm = \Yii::$app->getModule('community');
        if (!empty($moduleComm) && !empty($id)) {
            $community = \open20\amos\community\models\Community::findOne($id);
            if (!empty($community)) {
                if ($community->community_type_id == 1) {
                    $ret = true;
                }
            }
        }
