...
class TextEditorWidget extends TinyMce {
    ...
    protected function registerClientScript() {
        ...
        \dosamigos\tinymce\TinyMceAsset::register($view);
        TnyMentionAsset::register($view);
        ...
        if ($this->language !== null && $this->language !== 'en') {
            ...
            if (!empty($this->language)) {
                $langPathPlatform = '/tinymce/langs/' . $this->language . '.js';
                $languagePlatformFile = \Yii::getAlias('@webroot') . '/tinymce/langs/' . $this->language . '.js';
                if (file_exists($languagePlatformFile)) {
                    $this->clientOptions['language_url'] = \yii\helpers\Url::to($langPathPlatform, true);
                }
            }
        ...
        $options = \yii\helpers\Json::encode($this->clientOptions);
        $this->view->registerJsVar('config_comment_reply',$this->clientOptions);
...
JS;
        if (!empty($this->tinyMCELabel)) {
            $tinyLabelCharsCounter = <<<JS
                // Controllo l'esistenza della variabile prima di definirla, altrimenti crea errore come nel task 28257
                if (typeof charsCounterLabel === 'undefined') {
                    let charsCounterLabel = '';
                }
                charsCounterLabel = '$this->tinyMCELabel';
JS;
...
JS;
    ...
    protected function getLanguage() {
        ...
        $langPathPlatform = '/tinymce/langs/' . $langCode . '.js';
        $languagePlatformFile = \Yii::getAlias('@webroot') . '/tinymce/langs/' . $langCode . '.js';
        if (file_exists($languagePlatformFile)) {
            return $langCode;
        } else if (file_exists("{$languagePath}/langs/{$langCode}.js")) {
            return $langCode;
        } else {
