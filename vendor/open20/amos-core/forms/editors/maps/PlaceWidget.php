...
class PlaceWidget extends Widget
{
    ...
    /**
     * If true, the widget will hide the other locations markers
     * @var bool $hideOtherLocations
     */
    public $hideOtherLocations = false;
    ...
    public function init()
    {
        ...
        if (!empty($this->options['hideOtherLocations'])) {
            $this->hideOtherLocations = $this->options['hideOtherLocations'];
        }
...
function initMap_{$this->attribute}() {
    ...
    map_{$this->attribute} = new google.maps.Map(document.getElementById('map-{$this->attribute}'), {
        ...
        zoom: 10,
        styles: '{$this->hideOtherLocations}' ? [{
            featureType: "poi",
            elementType: "labels",
            stylers:
                [
...
JS;
    ...
    public function run()
    {
        ...
        if (!empty($this->placeAlias)) {
            ...
            $place_hash_string = $this->model->{$this->attribute};
            $place_id = PlacesComponents::getGoogleResponseByPlaceId($place_hash_string, false);
            $address = PlacesComponents::getGoogleResponseByPlaceId($place_hash_string, true, true);
        }

        $output = Html::hiddenInput($this->name, $place_id, [
            'id' => 'place-id-' . $this->attribute,
        ]);

        $output .= Html::textInput('tmp-pac-input_' . $this->attribute, "", [
            'id' => 'pac-input-' . $this->attribute,
            'class' => 'controls hidden',
            'style' => $this->showInput ? '' : 'display:none',
        ]);

        $output .= Html::tag('div', BaseAmosModule::t('amoscore', 'Caricamento mappa in corso') . "<br /><div id=\"progress-" . $this->attribute . "\" class=\"progress\"><div class=\"progress-bar progress-bar-striped active\" role=\"progressbar\" aria-valuenow=\"100\" aria-valuemin=\"0\" aria-valuemax=\"100\" style=\"width:100%\"></div></div>", [
            'id' => 'map-loader-' . $this->attribute,
            'class' => 'map-loader'
        ]);
        if (!empty($address)) {
            $output .= Html::tag('p', $address);
        }
        ...
        if (isset($place_fake) && $place_fake) {
            $output .= Html::tag('div', BaseAmosModule::t('amoscore', '#place_widget_address_identification_error'), [
                'class' => 'place-alert'
            ]);
        }
