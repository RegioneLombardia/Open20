    ...
    geocoder.geocode({'placeId': placeId}, function(results, status) {
        if (status === 'OK') {
            if (results[0]) {
                ...
                infowindow = window["infowindow_"+attribute];
                infowindow.close();
            }
        }
    });
...
function updateInfoWindow(attribute, address){
    ...
    infowindow.setContent(address+"&nbsp;&nbsp;<button type='button' class='btn btn-primary btn-remove-poi' onclick='resetPlaceWidget(\""+attribute+"\")'>Rimuovi</button>");
