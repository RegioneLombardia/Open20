 ...
 * @package open20\amos\core\record
 */
abstract class ContentModel extends NotifyRecord implements ContentModelInterface, CrudModelInterface, SearchModelInterface,
    ContentModelSearchInterface, ModelImageInterface, FacilitatorInterface
{
    public $modelImage;
    public $attachments;
    ...
    public function getSearchQueryCwhDisabled($query) {
        ...
        if (isset($table->columns['data_pubblicazione'])) {
            ...
            if ($table->columns['data_pubblicazione']->type == 'datetime') {
                $query->andFilterWhere(['<=', 'data_pubblicazione', $nowHour]);
            } else {
        ...
        if (isset($table->columns['data_rimozione'])) {
            if ($table->columns['data_rimozione']->type == 'datetime') {
                $query->andFilterWhere(['>=', 'data_rimozione', $nowHour]);
            } else {
    ...
    public function search($params, $queryType = null, $limit = null, $onlyDrafts = false, $pageSize = null)
    {
        ...
        } else if (!empty($pageSize)) {
            $dp_params['pagination']['pageSize'] = $pageSize;
        }
    ...
    public function buildQuery($params, $queryType, $onlyDrafts = false)
    {
        ...
        $query = $this->additionalQueryTypes($query, $queryType, $cwhActiveQuery);
...
//            }
    ...
    public function setEventAfterCounter()
    {
        Event::on(
            ...
            \open20\amos\core\widget\WidgetIcon::EVENT_AFTER_COUNT, [$this, 'notificationOffAfterCount']
        );
    }
...
//        die;
    ...
    /**
     * Checks if the content is a network content
     * @param string $networkType
     * @return bool
     */
    public function isNetworkContent($networkType = 'community')
    {
        $moduleCwh = \Yii::$app->getModule('cwh');
        if (!empty($moduleCwh)) {
            ...
            if (!empty($destinatari)) {
                foreach ($destinatari as $network) {
                    if (strpos($network, $networkType . '-') !== false) {
                        return true;
                    }
                }
            }
