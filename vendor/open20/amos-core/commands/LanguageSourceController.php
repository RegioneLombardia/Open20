...
class LanguageSourceController extends Controller
{
    ...
    /** Parametro per aggiornare le traduzioni di un singolo modulo
     * @var null $moduleName
     */
    ...
    public function actionLoadFiles()
    {
        foreach ($this->conf_path as $path) {
            ...
            if($realPath) {
                ...
                foreach ($modules as $key => $module) {
                    ...
                    if($mod && (is_null($this->moduleName) || (!is_null($this->moduleName) && $this->moduleName === $mod->getAmosUniqueId()))) {
        ...
        if ($i18nComponent) {
            ...
            foreach ($confvars as $extraCategory => $extraPath) {
                if(is_null($this->moduleName) || (!is_null($this->moduleName) && $this->moduleName === $extraCategory)) {
