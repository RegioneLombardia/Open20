    ...
    protected function renderDataCellContent($model, $key, $index) {
        ...
        if ($newActionColumn) {
            ...
            if (!empty(trim($html))) {
                ...
                $queryModal = "//div";
                $modals = $xpath->query($queryModal);

                foreach ($modals as $k => $md) {
                    $i = 0;
                    foreach ($md->attributes as $k2 => $attrM) {
                        if (strpos($attrM->nodeValue, 'modal') !== false && $i == 0) {

                            $div = $modals->item($k);
                            $div->setAttribute('style', "display: none;");
                            $htmlModal .= $doc->saveHTML($div);
                ...
                return $htmlModal . '<div class="dropdown bk-elementActions container-action">
