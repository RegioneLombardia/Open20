...
class CloudAssetManager extends AmosAssetManager {
    ...
    protected function publishDirectoryConsole($src, $options) {
        print_r("\n" . $src . "\n");
        $dir = $this->hash($src);
        print_r("\n" . $dir . "\n");

        $allDir = [];
        $allDir[] = \Yii::getAlias('@frontend/web/assets') . DIRECTORY_SEPARATOR . $dir;
        //DA GESTIRE CON CONFIGURAZIONI DI BACKEND NELL'EVENTUALITA' SI VOGLIA ATTIVARE
        //$allDir[] = \Yii::getAlias('@backend/web/assets'). DIRECTORY_SEPARATOR . $dir;
        foreach ($allDir as $dstDir) {
            if ($this->linkAssets) {
                if (!is_dir($dstDir)) {
                    FileHelper::createDirectory(dirname($dstDir), $this->dirMode, true);
                    try { // fix #6226 symlinking multi threaded
                ...
                print_r("\nCopy directory: " . $dstDir . "\n");
                FileHelper::copyDirectory($src, $dstDir, $opts);
            }

            $this->publishDirectoryS3Console($dstDir, $options);
        }
    }
    ...
    public function loadBundleConsole($name, $config = [], $publish = true) {
        if (!isset($config['class'])) {
            $config['class'] = $name;
        }
        /* @var $bundle AssetBundle */
        ...
        if ($publish && !empty($bundle->sourcePath)) {
            $this->publishConsole($bundle->sourcePath, $config);
        }

        return $bundle;
    }

    public function publishConsole($path, $options = []) {
        $path = \Yii::getAlias($path);

        if (!is_string($path) || ($src = realpath($path)) === false) {
            print_r("Errore publishConsole - The file or directory to be published does not exist: $path");
        } else if (!is_readable($path)) {
            print_r("Errore publishConsole - The file or directory to be published is not readable: $path");
        } else if (is_file($src)) {
            return $this->_published[$path] = $this->publishFile($src);
        } else {
            try {

                $this->publishDirectoryConsole($src, $options);
            } catch (\Exception $e) {
                print_r('Errore publishConsole: ' . $src . "\n" . $e->getMessage() . "\n" . $e->getTraceAsString() . "\n");
            }
        }
    }
    ...
    protected function publishDirectoryS3Console($src, $options) {
        try {
            $dir = \yii\helpers\StringHelper::basename($src);
            $baseAssetFrontend = \Yii::getAlias('@frontend/web/assets');
            $baseAssetBackend = \Yii::getAlias('@backend/web/assets');
            $baseAssets = [];
            $baseAssets[] = $baseAssetFrontend . DIRECTORY_SEPARATOR . $dir;
            //DA GESTIRE CON CONFIGURAZIONI DI BACKEND NELL'EVENTUALITA' SI VOGLIA ATTIVARE
            //$baseAssets[] = $baseAssetBackend . DIRECTORY_SEPARATOR . $dir;
            ...
            foreach ($baseAssets as $dstDir) {
                ...
                if (is_dir($dstDir)) {
                    ...
                    foreach ($filesConvert as $fileC) {
                        ...
                        $converter = $this->getConverter();
                        if (!empty($converter) && in_array(trim($dstFileC, DIRECTORY_SEPARATOR), self::$allValidCss)) {
                            $converter->convert(trim($dstFileC, DIRECTORY_SEPARATOR), $dstDir);
                        }
                    }
                    ...
                    $files = FileHelper::findFiles($src, array_merge($this->filterFilesOptions, $options));
                    $dstS3 = str_replace([\Yii::getAlias('@backend/web/'), \Yii::getAlias('@frontend/web/')], '', $dstDir);
                    $meta = $this->getMetaFromRemoteData($this->filesystem->listContents($dstS3, true), $dir);
                    ...
                    foreach ($directories as $directory) {
                        ...
                        $dirMeta = isset($meta[$key]) ? $meta[$key] : null;
                        if (!isset($dirMeta)) {
                            $this->filesystem->createDir($dstS3 . $dstDirectory);
                        }
                    }
                    ...
                    foreach ($files as $file) {
                        ...
                        try {
                            if (!isset($dirMeta[$dstBaseFile])) {
                                $this->filesystem->writeStream($dstS3 . $dstFile, fopen($file, 'r'));
            ...
            print_r('Errore in publishDirectoryS3Console: ' . $e->getMessage() . "\n");
        }
    ...
    public function setCompleted($finalKey, $app = null) {
        $path = $this->getPathCompleted($finalKey, $app);
        $h = fopen($path, 'w');
     ...
     * @return boolean
     */
    public function getCompleted($finalKey, $app = null) {
        $path = $this->getPathCompleted($finalKey, $app);
        if (file_exists($path)) {
            return true;
        }
     ...
     * @return type
     */
    protected function getPathCompleted($finalKey, $app = null) {
        if (!empty($app)) {
            return realpath(\Yii::getAlias('@' . $app . '/web/assets')) . DIRECTORY_SEPARATOR . $finalKey . '.pid';
        }
