        ...
        try {
            if (!\Yii::$app->request->isAjax) {
                if (!empty($params['model']) && !($params['model'] instanceof \stdClass) && is_object($params['model'])) {
                    if ($params['model']->hasMethod('getMetaTitle') && !empty($params['model']->getContentSeoData()->meta_title)) {
                        $this->seoTitle = $params['model']->getContentSeoData()->meta_title;
                    } else {
                        if ($params['model']->hasMethod('getOgTitle') && !empty($params['model']->getOgTitle())) {
                            $this->seoTitle = $params['model']->getOgTitle();
                        }
                    }
