...
class WorkflowTransitionStateDescriptorWidget extends Widget
{
    ...
    public function run()
    {
        ...
        if ($this->model->hasWorkflowStatus()) {
            ...
            if(!empty($this->forceStatusLabel) && !empty($this->model) && !empty($this->model->getWorkflowStatus()) && !empty($this->model->getWorkflowStatus()->getId())) {
                if(array_key_exists($this->model->getWorkflowStatus()->getId(), $this->forceStatusLabel)) {
                    $status = $this->forceStatusLabel[$this->model->getWorkflowStatus()->getId()];
                }
            }
