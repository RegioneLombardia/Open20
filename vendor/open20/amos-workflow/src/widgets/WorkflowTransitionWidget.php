            ...
            if ($state && ($key != $inState)) {
                if ($User->can($state->getId(), ['model' => $this->model])) {
                    ...
                    if ((!isset($metadati['hidden']) || (isset($metadati['hidden']) && strtolower($metadati['hidden']) != 'true')) && !$hiddenRoles) {
                        ...
                        $buttonsArr[] = [
                            'html' => '<div>' . (isset($metadati[$currentStatus . '_description']) ? $module::t($this->translationCategory, $metadati[$currentStatus . '_description']) : ((isset($metadati['description'])) ? $module::t($this->translationCategory, $metadati['description']) : '')) . CloseSaveButtonWidget::widget([
                                    'model' => $this->model,
