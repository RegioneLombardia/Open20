...
<?php
if(!empty($generator->workflow)){
    ?>
const TR_WORKFLOW = '<?= $generator->workflow ?>';
<?php
}
$arrayAttributi = [];
$indx = [];
...
public function behaviors() {
        return ArrayHelper::merge(parent::behaviors(), [
                    [
                        ...
                        'defaultWorkflowId' => self::TR_WORKFLOW,
...
<?php foreach ((array) $elencoAttributi as $key => $value):
    ?>   
    '<?= lcfirst(Inflector::id2camel($key, '_')) ?>' => \Yii::t('amostranslation', '<?= ucfirst(addslashes($value['descriptorField'])) ?>'),<?= "\n" ?>
    'attr<?= Inflector::id2camel($key, '_') ?>Mm' => \Yii::t('amostranslation', '<?= ucfirst(addslashes($value['descriptorField'])) ?>'),<?= "\n" ?>
    <?php
endforeach;
?>
];
