...
$hideViewTabs = $eventsModule->hideViewTabs;
...
//                'imageUrl'      => !empty($model->getDiscussionsTopicImage()) ? $model->getDiscussionsTopicImage()->getWebUrl('square_small') : '',
                ...
                <div class="box col-xs-12 nop">
                    ...
                    <div class="col-xs-12">
                        ...
                        $showButton = (
                            ...
                            (
                                (!$eventsModule->enableAutoInviteUsers && ($model->eventType->event_type != EventType::TYPE_UPON_INVITATION)) ||
                                ($eventsModule->enableAutoInviteUsers && !is_null($loggedUserRegisteredInvitation) && ($model->eventType->event_type != EventType::TYPE_INFORMATIVE))
                            )
    ...
    <?php
    if (!in_array('tab-overview', $hideViewTabs)) {
        $itemsTab[] = [
    ...
    if (\Yii::$app->user->can('ADMIN')) {
        ...
        <?php
        if (!in_array('tab-organization', $hideViewTabs)) {
            $itemsTab[] = [
    ...
    if (!empty($model->eventAttachments)) {
        ...
        <?php
        if (!in_array('tab-attachments', $hideViewTabs)) {
            $itemsTab[] = [
    ...
    <?php if ($communityPresent && $model->validated_at_least_once): ?>
        ...
        <?php
        if (!in_array('tab-participants', $hideViewTabs)) {
...
//                        ],
        ...
        <?php $this->endBlock();
        if (!in_array('tab-calendars', $hideViewTabs)) {
            $itemsTab[] = [
        ...
        if (!in_array('tab-mybooking', $hideViewTabs)) {
            $itemsTab[] = [
    ...
    <?php if ($model->isSeatManagement() && EventsUtility::hasPrivilegesLoggedUser($model)) { ?>
        ...
        <?php
        if (!in_array('tab-seats_management', $hideViewTabs)) {
            $itemsTab[] = [
    ...
    <?php if ($communityPresent && (($model->status != Event::EVENTS_WORKFLOW_STATUS_DRAFT) || $model->validated_at_least_once)): ?>
        ...
        <?php
        if (!in_array('tab-staff', $hideViewTabs)) {
    ...
    <?php if ($eventsModule->enableNewWizard) { ?>
        <?= \yii\helpers\Html::a(AmosEvents::t('amosevents', 'Close'),['/events/event-dashboard/view', 'id' =>$model->id ], [
