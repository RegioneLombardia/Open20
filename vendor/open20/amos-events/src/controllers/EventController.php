...
class EventController extends base\EventController {
    ...
    public function behaviors() {
        ...
        if ($this->eventsModule->openEventsToGuests) {
            $rules[] = [
                'allow' => true,
                'actions' => [
                    ((!empty(\Yii::$app->params['befe']) && \Yii::$app->params['befe'] == true) ? 'index' : 'nothingindex'),
                    ((!empty(\Yii::$app->params['befe']) && \Yii::$app->params['befe'] == true) ? 'all-events' : 'nothingallevents'),
                    ((!empty(\Yii::$app->params['befe']) && \Yii::$app->params['befe'] == true) ? 'view' : 'nothingread')
                ],
                'matchCallback' => function ($rule, $action) {
                    if (in_array($action->id, ['index', 'all-events'])) {
                        return true;
                    }
                    if ($action->id != 'view') {
                        return false;
    ...
    public function actionAllEvents() {
        ...
        $this->setDataProvider($this->modelSearch->searchCalendarViewAll(Yii::$app->request->getQueryParams()));

        if (empty(\Yii::$app->request->get('currentView'))) {
            $this->setCurrentView($this->getAvailableView($this->amosEventsModule->defaultViewAllEvents));
        }
    ...
    public function actionImportInvitations($id) {
        ...
        if (Yii::$app->request->post() && $upload->load(Yii::$app->request->post())) {
            ...
            if ($rows === false) {
                ...
                $registerSendDatetime = false;
                if ($this->eventsModule->saveExternalInvitations) {
                    $rsp = $this->doImportInvitations($id, $rows);
                    if ($rsp['cnt'] > 0) {
                        $sendInvitations = true;
                        $event = $this->findModel($id);
                        $rows = $event->getInvitationsData(true);
                        $registerSendDatetime = true;
                    }
                }
                if ($sendInvitations) {
                    $rsp = $this->doSendInvitations($id, $rows, $registerSendDatetime); // Send invitations only
                }
    ...
    public function actionEventSignup($eid, $pName = null, $pSurname = null, $pEmail = null, $emptyFields = false, $pCode = null, $isGroup = false) {
        ...
        $multipleRecording = $this->eventsModule->multipleRecording || (isset($this->eventsModule->params['multiple_event_signup']) && $this->eventsModule->params['multiple_event_signup']);
                ...
                if ($event->event_type_id != EventType::TYPE_LIMITED_SEATS ||
                    ...
                    if (\Yii::$app->request->post()) {
                        ...
                        if (array_key_exists('EventParticipantCompanionDynamic', $post)) {
                            $companionsData = MultipleModel::createMultiple($this->eventsModule->createModel('EventParticipantCompanionDynamic'), [], ['event_id' => $eid]);
                        ...
                        if ($eventParticipantModel->validate() && $modelValidations) {
                            if ((count($invitationFound) == 0) || ($this->eventsModule->enableAutoInviteUsers && (count($invitationFound) > 0))) {
                                ...
                                if ($gdprMarked) {
                                    ...
                                    if ($event->event_type_id == EventType::TYPE_LIMITED_SEATS &&
                                        ...
                                        $participant = $this->addParticipant($eid, $post['EventParticipantCompanion'], $user->id, $gdprQuestions, $invitation);
...
// Saving the document as OOXML file...
    ...
    public function actionJsoncalendar($start = NULL, $end = NULL, $_ = NULL) {
        ...
        $search = $this->getModelSearch()->searchManagement(Yii::$app->request->getQueryParams());
        $agenda = $search->query->andWhere(new \yii\db\Expression("(DATE(begin_date_hour) BETWEEN '$startF' AND '$endF') OR (DATE(end_date_hour) BETWEEN '$startF' AND '$endF')"))->all();
        ...
        foreach ($agenda AS $time) {
            ...
            $Event->id = $time->id;
            $Event->title = $time->getEventTitle();
//            $Event->nonstandard = [
//                'description' => 'test',
//            ];
            $Event->start = $time->begin_date_hour;
            $Event->end = $time->end_date_hour;
            $Event->color = $time->getEventColor();
            $Event->url = $time->getFullViewUrl();
            $events[] = $Event;
        }
