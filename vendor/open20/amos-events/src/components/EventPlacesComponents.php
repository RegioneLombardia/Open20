...
class EventPlacesComponents
{
    ...
    public static function getGoogleResponseByPlaceId($place_id, $trySave = false)
    {
        ...
        if ($ResultGeocoding && isset($ResultGeocoding['status'])) {
            if ($ResultGeocoding['status'] == 'OK') {
                if ($trySave) {
                    self::saveLocationPlace($place_id, $ResultGeocoding, 'place');
                }
    ...
    public static function getGoogleResponseByGeocodeString($geocodeString, $trySave = false)
    {
        ...
        if ($ResultGeocoding && isset($ResultGeocoding['status'])) {
            if ($ResultGeocoding['status'] == 'OK') {
                ...
                if ($trySave) {
                    self::saveLocationPlace($place_id, $ResultGeocoding, 'geocode');
                }
     ...
     * @throws \yii\base\InvalidConfigException
     */
    public static function saveLocationPlace($place_id, $ResultGeocoding, $from = 'geocode')
    {
        /** @var OrganizationsPlaces $model */
        $model = EventPlaces::className();
