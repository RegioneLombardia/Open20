 ...
 * @package open20\amos\news\widgets\icons
 */
class WidgetIconEventOperators extends WidgetIcon
{
    /**
     * @inheritdoc
     */
    public function init()
    {
        ...
        $this->setLabel(AmosEvents::tHtml('amosevents', 'Operatori'));
        $this->setDescription(AmosEvents::t('amosevents', 'Operatori'));
        ...
        if($group) {
            $this->setUrl(['/events/event-group-referent-mm/index', 'group_id' => $group->id]);
        }
        else {
            $this->setUrl(['/']);
        }
