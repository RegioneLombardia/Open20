...
JS;
    ...
    public function run()
    {
        ...
        if ($place_fake) {
            $geocodeString = EventPlacesComponents::getGeocodeString($placeDataObj);
            if ($geocodeString) {
                $tmp_place_id = EventPlacesComponents::getGoogleResponseByGeocodeString($geocodeString, true);
                if ($tmp_place_id) {
        ...
        return
            ...
            Html::tag('div', Yii::t('amosorganizzazioni', 'Caricamento mappa in corso') . "<br /><div id=\"progress-" . $this->attribute . "\" class=\"progress\"><div class=\"progress-bar progress-bar-striped active\" role=\"progressbar\" aria-valuenow=\"100\" aria-valuemin=\"0\" aria-valuemax=\"100\" style=\"width:100%\"></div></div>", [
            ...
            ($place_fake
                ?
                "<div class='place-alert'>" . Yii::t('amosorganizzazioni', 'E\' stato rilevato un problema nell\'identificazione dell\'indirizzo. Prego verificare ed eventualmente correggere l\'indirizzo identificato automaticamente dal sistema.') . "</div>"
                :
                "");
    }
}