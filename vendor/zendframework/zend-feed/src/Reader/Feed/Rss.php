...
class Rss extends AbstractFeed
{
    ...
    public function getCopyleft()
    {
        ...
        if ($this->getType() !== Reader\Reader::TYPE_RSS_10 &&
            $this->getType() !== Reader\Reader::TYPE_RSS_090) {
            $copyleft = $this->xpath->evaluate('string(/rss/channel/copyleft)');
        }

        if (! $copyleft && $this->getExtension('DublinCore') !== null) {
            $copyleft = $this->getExtension('DublinCore')->getCopyleft();
        }

        if (empty($copyleft)) {
            $copyleft = $this->getExtension('Atom')->getCopyleft();
        }
